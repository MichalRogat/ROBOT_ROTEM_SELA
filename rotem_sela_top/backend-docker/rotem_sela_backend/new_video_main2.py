...
def videoFeedHandler(port, cam_id, queue, barrier, txqueue:multiprocessing.Queue()):
        global isMain
        isMain = False
            ...
            with v4l2py.Device(video_dev) as device:
                ...
                for frame in device:
                    try:
                        ...
                        try:
                            txqueue.put({
                                port:res[cam_id[sideIdx][camIdx]]['dev']
                            })
...
if __name__ == "__main__":
    ...
    for item in CAM_PORTS:
        ...
        txqueue = multiprocessing.Queue()
        process = multiprocessing.Process(target=videoFeedHandler, args=(item[1], item[0], queue, barrier, txqueue))
        processes.append(process)
        subQueues.append(queue)
        subTxQueue.append(txqueue)
        process.start()
